...
/**
 * This is a model for the Bird Watching Game that contains all objects in the
 * game. This Model is responsible for updating the movement of the objects,
 * collision detection, and updating other attributes of objects within the
 * model.
 * 
 * @author Tyler
 */
public class BirdWatchingGameModel extends Model {
	...
	/**
	 * Creates an instance of the BirdWatchingGameModel class. This constructor
	 * creates an array to hold each of the three birds of the Bird class. The
	 * constructor also chooses a random bird to assign to the variable
	...
	/**
	 * Checks to see if a given bird is within the bounds of the screen
	 * depending on the direction that the bird is moving.
	 * 
	 * @param b
	 *            The bird to check the position of
	 */
	 ...
	 * 
	 * @param c
	 *            the Camera to check the position of
	 */
	 ...
	 * picture of the bird they are looking for, the incorrect bird, or just the
	 * background.
	 * 
	 * @param target
	 *            The Bird that the player is searching for.
	 * @param birdList
	 *            The list of birds within the model.
	 */
		...
		if (tutorial) {
			if (!birds.isEmpty()) {
				...
				if (isOnTarget(target)) {
					...
					Random rand = new Random();
					searchingFor = birds.get(rand.nextInt(numBirdsNotFound)); // change
																				// target
																				// bird
					tutorial = false;
				}
