...
/**
 * <h1>ResearchGameModel Class</h1> Going by the MVC design pattern, this is the
 * Model class. This is more or less the brain of the ResearchGame. Everything
 * that is needs to be updated is calcualted here and sent over to the view to
 * paint. This class is fed by the controller, with each key press of the game
 * controls the players direction is updated in conrtoller and sent here to this
 * class to do the game logic. The main methods are multiple collision
 * detections like player with crabs and player with out-of-bounds, and the
 * update location which updates the position of everything that moves and sent
 * to View.
 * <p>
 * 
 * @author mattstack
...
public class ResearchGameModel {
	...
	/**
	 * <h1>ResearcherGameModel Constructor</h1> Constructs the Model. Takes in a
	 * width, height, imageWidth, and imageHeight from the views knowledge of
	 * screen size and image (sprite) size. Outside of this is creates crabs and
	 * adds them to the crabs array and creates rectangles for boundaries and
	 * add those to the rectangle array, both with specific placement
	 * 
	 * @param width
	 *            width of the frame, passed in from View
	 * @param height
	 *            height of the frame, passed in from the View
	 * @param imageWidth
	 *            width of the images
	 * @param imageHeight
	...
	public ResearchGameModel(int width, int height, int imageWidth, int imageHeight, Boolean tutorial) {
		...
		player = new Researcher((int) (playerStartingX * scaleFacX), (int) (playerStartingY * scaleFacY), startDir,
				startingLives);
		if (tutorial == false) {
	...
	/**
	 * <h1>getCrabs</h1> Returns the array of Crabs create from the Model
	 * constructor
	 * 
	...
	/**
	 * <h1>getRects</h1> Returns the array of Rectangle that represent the
	 * out-of-bounds to pass into the collision detector
	 * 
	...
	/**
	 * <h1>crabCollisionChecker</h1> Checks if the any of the crab's rectanclges
	 * from any crab on the list is intersecting the players rectangle. If so,
	 * then subtract a life point from player if the crab has not been steppedOn
	 * before, and set that crabs steppedOn to be true
	 */
	...
	/**
	 * <h1>boundsCollison</h1> Checks if the player rectangle is intersecting
	 * any of the out-of-bounds rectangles, and if so, then subtract all the
	 * points and make the game start over
	 */
	...
	/**
	 * <h1>endCheck</h1> Checks to see if the player has gotten to the end of
	 * the game and will be sent back to the main screen, returns true if so
	 * 
	 * @return boolean value of it game is over (true) and should be brought
	 *         back to main screen
	 */
	public Boolean endCheck() {
		if (player.getxPos() >= getScaledWidth(5500)) {
			player.direction = RDirection.IDLE;
			return true;
		} else {
			return false;
		}
	...
	/**
	 * <h1>tutorialEndCheck</h1> Checks to see if the player has reached the end
	 * of the tutorial and it should restart
	 * 
	 * @return boolean value of the player has reached the end of the tutorial
	...
	/**
	 * <h1>lifeCheck</h1>Checks if the player has run out of lives, and if so,
	 * restart the game
	 * 
	 * @return whether True if the player has greater than 0 lives and False if
	 *         not
	 */
	...
	/**
	 * <h1>tutorialLifeCheck</h1>Checks if the player has run out of lives, and
	 * if so, restart the tutorial
	 * 
	 * @return whether True if the player has greater than 0 lives and False if
	 *         not
	 */
	...
	/**
	 * <h1>updateLocationAndModel</h1> Handles all of the location changing, all
	 * depending on the direction of the player (from Controller). Updates all
	 * crabs positions and rectangle, player position, and bounds rectangles.
	 */
	public void updateLocationAndDirection() {
		// System.out.println("x: " + player.getxPos() + ", y: " +
		// player.getyPos()); // print x and y of player
