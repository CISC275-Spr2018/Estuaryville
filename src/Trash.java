...
public class Trash extends Mover {
	 ...
	 */
	public enum Type {
		CAN("can"), PLASTIC("plastic"), STRAW("straw");
		 ...
		 * 
		 * @param s
		 *            a String which is a partial directory of the Type (for
		 *            loading images)
		 */
	...
	/**
	 * Creates a new piece of Trash with random off-screen starting x and y
	 * position and direction
	 * 
	 * @param t
	 *            a Type, a straw, can, or plastic
	 */
	public Trash(Type t) {
		if (Math.random() > 0.5) {
		...
		originY = (int) (Math.random()
				* (FishingGameModel.WATER_BOTTOM - FishingGameModel.WATER_TOP - FishingGameView.TRASH_HEIGHT - 50)
				+ FishingGameModel.WATER_TOP + 50);
		super.setYPos(originY);
		super.setXSpeed((int) (Math.random() * 13) + 7);
		trashType = t;
		hooked = false;
		hookedFor = 0;
		hitbox = new Rectangle(this.getXPos(), this.getYPos(), FishingGameView.TRASH_WIDTH,
				FishingGameView.TRASH_HEIGHT);
	}
	 ...
	 * 
	 * @param b
	 *            a boolean representing if the trash is hooked
	 */
